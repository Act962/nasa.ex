/**
 * Handler agnóstico de framework do webhook do Instagram DM (verificação Meta +
 * inbound de DM → lead). A Meta NÃO assina este endpoint (lookup por
 * instagram_account_id); recebemos o corpo cru e parseamos aqui. Compartilhado
 * entre o route Next (apps/web) e a rota Fastify (apps/api).
 */
...
export async function handleInstagramVerify(params: {
  mode: string | null;
  token: string | null;
...
export async function handleInstagramEvent(rawBody: string): Promise<WebhookResult> {
  try {
    ...
    for (const entry of body.entry ?? []) {
      for (const event of entry.messaging ?? []) {
        ...
        if (!lead) {
          ...
          const igReferral = await resolveReferralForOrg(
          ...
          lead = await prisma.lead.create({
            data: {
              ...
              ...(igReferral
                ? ctwaToLeadData(igReferral.ref, igReferral.resolved)
                : {}),
              conversation: {
                create: {
                  remoteJid,
        ...
        await trackLeadEvent({
          leadId: lead!.id,
          kind: "message_in",
          metadata: { channel: "INSTAGRAM", messageId },
        });
