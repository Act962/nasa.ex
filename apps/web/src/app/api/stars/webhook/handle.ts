/**
 * Handler agnóstico de framework do webhook dedicado de recarga de Stars via
 * Stripe (secret `STRIPE_STARS_WEBHOOK_SECRET`, Stripe do sistema). Recebe o
 * corpo CRU + assinatura e devolve `{ status, body }`. Compartilhado entre o
 * route Next (apps/web) e a rota Fastify (apps/api).
 */
...
export async function handleStarsWebhook(
  rawBody: string,
  signature: string,
  ...
  try {
    ...
    console.error("[stars/webhook] signature error:", message);
    ...
    switch (event.type) {
      ...
      case "payment_intent.succeeded": {
        ...
        let starsPaymentId: string | null =
          paymentIntentMetadata.kind === "stars_topup"
            ? paymentIntentMetadata.starsPaymentId ?? null
            : null;
        if (!starsPaymentId) {
          const starsPayment = await prisma.starsPayment.findFirst({
            where: { stripePaymentIntentId: paymentIntent.id, provider: "stripe" },
            select: { id: true },
          });
          starsPaymentId = starsPayment?.id ?? null;
        }
      ...
      case "charge.refunded": {
        ...
        if (!isFullRefund) {
          console.log(
            `[stars/webhook] charge.refunded parcial pi=${paymentIntentId} ${charge.amount_refunded}/${charge.amount} — só log.`,
          );
          break;
        }

        const starsPayment = await prisma.starsPayment.findFirst({
          where: { stripePaymentIntentId: paymentIntentId, provider: "stripe" },
          select: { id: true },
        });
        if (!starsPayment) break; // refund alheio (curso/plano) — ignore.
        ...
        try {
          const { revertedNow, reverted } = await prisma.$transaction((tx) =>
            revertStarsTopUpInTx({
              ...
              eventId: event.id,
              eventType: event.type,
              starsPaymentId: starsPayment.id,
              reason: `Reembolso Stripe (charge=${charge.id})`,
            }),
          );
          console.log(
            `[stars/webhook] charge.refunded ${revertedNow ? "aplicado" : "no-op"}: payment=${starsPayment.id} estorno=${reverted}★`,
          );
        } catch (err) {
