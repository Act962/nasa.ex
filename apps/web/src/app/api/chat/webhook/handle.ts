// Handler agnóstico de framework do webhook Uazapi (inbound WhatsApp principal).
// Recebe trackingId (da query) + corpo cru e devolve um `Response` Web-standard,
// consumível tanto pelo route Next (apps/web) quanto pela rota Fastify (apps/api).
...
export async function handleUazapiWebhook(
  ...
  try {
    ...
    if (base.data.EventType === "calls") {
      ...
      return Response.json({ success: true, status: mappedStatus }, { status: 201 });
    }
    ...
    return Response.json({ error: "Event not handled" }, { status: 404 });
  } catch (error: any) {
