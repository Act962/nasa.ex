/**
 * Seed de Relatórios Meta Ads — 90 Dias
 *
 * Variante do `seed-meta-ads-demo.ts` que popula 90 dias de snapshots em vez
 * de 30. Útil pra testar filtros de período longo e gráficos de série temporal.
 *
 * Estrutura criada (prefixo `seed-meta-90d-` pra coexistir com o seed de 30d):
 *  - 1 ad account fake
 *  - 4 campanhas
 *  - 8 adsets (2 por campanha)
 *  - 16 ads (2 por adset)
 *  - Snapshots × 90 dias × 4 níveis ≈ 29 entidades × 90 dias = 2.610 snapshots
 *
 * Rode com: npx tsx prisma/seed-meta-ads-90d.ts
 *
...
const SEED_PREFIX = "seed-meta-90d-";
...
const CAMPAIGNS: SeedCampaign[] = [
  {
    metaCampaignId: `${SEED_PREFIX}cmp-vendas-trimestre`,
    name: "Vendas Trimestre — Conversões",
  ...
  {
    metaCampaignId: `${SEED_PREFIX}cmp-leads-funil`,
    name: "Funil de Leads — Captação",
    objective: "OUTCOME_LEADS",
    baseImpressions: 35_000,
    baseSpend: 220,
    baseConvRate: 0.028,
    hasVideo: true,
  },
  {
    metaCampaignId: `${SEED_PREFIX}cmp-awareness-trimestre`,
    name: "Awareness Trimestre — Topo de funil",
    objective: "OUTCOME_AWARENESS",
    baseImpressions: 110_000,
    baseSpend: 250,
    baseConvRate: 0.006,
    hasVideo: true,
  },
  {
    metaCampaignId: `${SEED_PREFIX}cmp-retention`,
    name: "Retenção — Reengajar Base",
    objective: "OUTCOME_TRAFFIC",
    baseImpressions: 25_000,
    baseSpend: 150,
    baseConvRate: 0.022,
    hasVideo: false,
  },
...
const ADSETS_BY_CMP: Record<string, { name: string; goal: string; weight: number }[]> = {
  [`${SEED_PREFIX}cmp-vendas-trimestre`]: [
    { name: "Lookalike Compradores 1%", goal: "OFFSITE_CONVERSIONS", weight: 0.6 },
    { name: "Retargeting 30d", goal: "OFFSITE_CONVERSIONS", weight: 0.4 },
  ],
  [`${SEED_PREFIX}cmp-leads-funil`]: [
    { name: "Interesse Mercado-Alvo", goal: "LEAD_GENERATION", weight: 0.55 },
    { name: "Lookalike Leads Anteriores", goal: "LEAD_GENERATION", weight: 0.45 },
  ],
  [`${SEED_PREFIX}cmp-awareness-trimestre`]: [
    { name: "Broad Demográfico", goal: "REACH", weight: 0.7 },
    { name: "Engajadores 90d", goal: "REACH", weight: 0.3 },
  ],
  [`${SEED_PREFIX}cmp-retention`]: [
    { name: "Visitantes Site 30d", goal: "LINK_CLICKS", weight: 0.5 },
    { name: "Engajadores Página 30d", goal: "LINK_CLICKS", weight: 0.5 },
  ],
};
...
async function main() {
  console.log(`\n🌱 Seed Meta Ads 90d — usuário: ${TARGET_EMAIL}\n`);
  ...
  console.log("🧹 Limpando seed 90d anterior...");
  ...
  for (let dayOffset = DAYS_BACK; dayOffset >= 1; dayOffset--) {
    ...
    let acc = generateKpis({ baseImpressions: 0, baseSpend: 0, baseConvRate: 0, hasVideo: false, weight: 0, multiplier: 0 });

    for (const { data: cmp } of createdCampaigns) {
      const k = generateKpis({ ...cmp, weight: 1, multiplier: mult });
    ...
    snapshotsToCreate.push({
      ...
      entityId: AD_ACCOUNT_ID, entityName: `Conta Demo 90d (${org.name})`, date, ...acc,
    });
    ...
    for (const adset of createdAdsets) {
      snapshotsToCreate.push({
        ...
        ...generateKpis({ ...adset.data, weight: adset.weight, multiplier: mult }),
      });
    }
    ...
    for (const ad of createdAds) {
      snapshotsToCreate.push({
        ...
        ...generateKpis({ ...ad.data, weight: ad.weight, multiplier: mult }),
      });
    }
  }
