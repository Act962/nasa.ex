    ...
    <div className="size-12 rounded-xl overflow-hidden flex items-center justify-center shrink-0 relative border border-border/50">
      ...
      {!imgFailed && (
        ...
        <img
          ...
          className={cn(
            "absolute inset-0 size-full object-contain bg-white p-1.5 transition-opacity duration-200",
...
function PaymentChoice({
  ...
  const { data: costData } = useQuery({
    ...orpc.stars.getAppCost.queryOptions({
      input: { appSlug: integration.slug },
    }),
    staleTime: 5 * 60 * 1000,
  });
          ...
          <div className="space-y-4 py-1">
            ...
            <div className="rounded-xl border bg-muted/30 p-4 space-y-2.5">
              ...
              <p className="text-xs text-muted-foreground pl-7 leading-relaxed">
                ...
                integração no seu painel. No próximo passo você inserirá as
...
function getAppPriceId(appSlug: string): string {
  const map: Record<string, string> = {
    "whatsapp-business":
      process.env.NEXT_PUBLIC_STRIPE_PRICE_WHATSAPP ?? "price_wpp_placeholder",
    "instagram-dm":
      process.env.NEXT_PUBLIC_STRIPE_PRICE_INSTAGRAM ?? "price_ig_placeholder",
    tiktok:
      process.env.NEXT_PUBLIC_STRIPE_PRICE_TIKTOK ?? "price_tiktok_placeholder",
    linkedin:
      process.env.NEXT_PUBLIC_STRIPE_PRICE_LINKEDIN ??
      "price_linkedin_placeholder",
  };
