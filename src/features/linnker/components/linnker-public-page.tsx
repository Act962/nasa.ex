...
export function LinnkerPublicPage({ page, isDraft = false }: Props) {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center relative overflow-x-hidden">
      {/* Background */}
      <div className="fixed inset-0 -z-10" style={{ background: page.backgroundColor ?? "#f3f4f6" }} />
      ...
      {/* Draft banner */}
      {isDraft && (
        <div className="w-full bg-yellow-400 text-yellow-900 text-xs font-semibold text-center py-2 px-4 relative z-10">
          ⚠️ Esta página está em rascunho — apenas você pode ver este preview
        </div>
      )}

      {/* Top banner image (above cover) */}
      {page.bannerUrl && (
      ...
      {/* Content */}
      <div className="w-full max-w-sm px-4 pt-16 pb-16 flex flex-col items-center gap-5">
        {/* Identity */}
        <div className="text-center">
          <h1 className="text-2xl font-bold" style={{ color: page.titleColor ?? "#111827" }}>{page.title}</h1>
          {page.bio && (
            <p className="mt-2 text-sm leading-relaxed" style={{ color: page.bioColor ?? "#6b7280" }}>{page.bio}</p>
          )}
        </div>
          ...
          {activeLinks.map((link) => {
            ...
            if (link.displayStyle === "banner") {
              return (
                <button
                  ...
                  className="w-full overflow-hidden shadow-md hover:opacity-95 hover:scale-[1.01] active:scale-[0.99] transition-all"
                ...
                >
                  {link.imageUrl ? (
                    <div className="relative">
                      ...
                      {link.title && (
                        <div
                          className="absolute bottom-0 left-0 right-0 px-4 py-2.5 text-white font-semibold text-sm"
                    ...
                    <div
                      className="w-full flex items-center justify-center gap-3 px-5 py-8 font-semibold text-white text-sm"
                      style={{ background: link.color ?? page.coverColor }}
                    >
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => handleLinkClick(link)}
                className="w-full flex items-center gap-3 px-5 py-3.5 font-semibold text-white shadow-md hover:opacity-90 hover:scale-[1.01] active:scale-[0.99] transition-all text-sm"
                style={{ background: link.color ?? page.coverColor, borderRadius: radius }}
              >
                {link.imageUrl ? (
                  <img src={link.imageUrl} alt="" className="size-7 rounded object-cover shrink-0" />
                ) : (
