...
5. **Modelo: OpenAI**, default `gpt-4.1-mini`, override via `ASTRO_DEFAULT_MODEL`. Reusa `OPENAI_API_KEY` (mesma do RAG/Astro). Histórico: começou em `gpt-4o-mini` (2026-05-18); trocado pra `gpt-4.1` em 2026-05-23 após relato de respostas ruins; ajustado pra `gpt-4.1-mini` em 2026-05-25 (custo/latência menores mantendo qualidade da família 4.1).
...
- `OPENAI_API_KEY` (obrigatória pra ligar a IA padrão — fallback quando o cliente não configura BYO)
- `ASTRO_DEFAULT_MODEL` (opcional, default `gpt-4o-mini`)
- `AI_SECRETS_KEY` (obrigatória se algum tracking configurar BYO API key — chave usada pelo AES-256-GCM em [src/lib/crypto.ts](../../lib/crypto.ts))
...
- [x] ~~**Duplicação consciente do dispatch de workflows LEAD_TAGGED**~~ — resolvido em 2026-05-23. Match centralizado em [`find-matching-workflows.ts`](../../triggers/components/lead-tagged/find-matching-workflows.ts), consumido pelos dois caminhos (router humano + IA).
...
- **2026-05-21** — Fix: respostas a botões/listas não apareciam na UI. O webhook [chat/webhook/route.ts](../../app/api/chat/webhook/route.ts) só persistia `ExtendedTextMessage`/`Conversation` e ignorava `TemplateButtonReplyMessage`/`ButtonsResponseMessage`/`ListResponseMessage`/`InteractiveResponseMessage` (caía no `!messageData` → `"Message type not processed"`). Esses tipos agora compartilham o mesmo upsert de texto, extraindo o body de `content.selectedDisplayText` / `content.selectedButtonId` / `content.title` / `message.vote` quando `text` vier vazio (como no payload de TemplateButtonReply do Uazapi: `text:""`, `vote:"Lavagem, Higienizaçã"`, `content.selectedDisplayText:"Lavagem, Higienizaçã"`).
- **2026-05-23** — Troca do modelo default de `gpt-4o-mini` → `gpt-4.1` em [model.ts](lib/model.ts). Motivo: qualidade ruim das respostas em atendimento (instruction-following fraco, tom genérico). `ASTRO_DEFAULT_MODEL` continua sobrescrevendo o default. Sem mudança de schema/contrato.
- **2026-05-25** — Default ajustado de `gpt-4.1` → `gpt-4.1-mini` em [model.ts](lib/model.ts). Mantém o instruction-following da família 4.1 com custo/latência mais baixos pra atendimento em rajadas. `ASTRO_DEFAULT_MODEL` continua sobrescrevendo.
- **2026-05-23** — Fix: `AI_InvalidPromptError: messages must not be empty`. O slate-reset por `settings.updatedAt` em [loadAgentContext](lib/context.ts) eliminava 100% do histórico quando `Message.createdAt <= settings.updatedAt` — race comum entre persistência da inbound no webhook e o `@updatedAt` do `AiSettings` (Prisma grava no commit), ou idle-reopen logo após editar o prompt. Agora o filtro tem **fallback determinístico**: se zerar tudo, usa o histórico completo. Adicionado também guard `empty_history` em [agent.ts](lib/agent.ts) pra cobrir conversas truly vazias (só mídia sem texto/caption) — evita chamar o SDK com `messages: []`.
- **2026-05-23** — Fix: dispatch de workflows `LEAD_TAGGED` falhando quando IA (ou humano) aplica múltiplas tags numa chamada. O `array_contains: input.tagIds` antigo usa `@>` no JSONB e exige que `workflow.action.tagIds` contenha TODAS as tags adicionadas — semântica de subset, não de overlap. Em casos típicos (ex: prompt "ao escolher menu, aplicar tag específica + tag 'Finalizado pelo Robô'") nenhum workflow casava, porque o trigger costuma ser configurado pra UMA das tags. Match agora vive em [`find-matching-workflows.ts`](../../triggers/components/lead-tagged/find-matching-workflows.ts) — uma query por tag, dedup por workflow id. Consumido por [`apply-tags-by-ai.ts`](lib/apply-tags-by-ai.ts) (IA) e [`router/leads/add-tags.ts`](../../../app/router/leads/add-tags.ts) (humano). Resolve a dívida "Duplicação consciente do dispatch de workflows LEAD_TAGGED". Também: removida menção a `finish_conversation` do `finishBlock` em [`system-prompt.ts`](lib/system-prompt.ts) (tool ficou desligada desde 2026-05-22 — decisão de produto: humano sempre assume no final). Taggingblock reforçado com regra "todas as tags relevantes na MESMA chamada" + few-shot de chamada combinada com `transfer_to_human`.
- **2026-05-26** — Telemetria de uso (tokens) por execução. Novo model `AiChatRun` (trackingId, organizationId, leadId, conversationId, provider, modelId, usingCustom, input/output/totalTokens, toolCalls, createdAt; índices em `[trackingId, createdAt]` e `[organizationId, createdAt]`). [agent.ts](lib/agent.ts) ganhou step `persist-usage` que grava uma linha por run usando `result.usage` do AI SDK; falha no insert apenas loga (não derruba o agente). Nova oRPC `ia.usage.get` em [`get-ai-usage.ts`](../../app/router/ia/get-ai-usage.ts) retorna totais (runs, input/output/totalTokens, toolCalls), série diária via `$queryRaw` (Postgres `date_trunc`) e últimas N runs. Nova aba "Uso" em [chatbot-ia-usage-tab.tsx](../tracking-settings/components/chatbot-ia-usage-tab.tsx) com cards de totais, mini-bar chart diário e tabela de execuções recentes. Sem custo em USD nesta fase (decisão consciente; fica no backlog). Migration anterior `add_ai_provider_to_ai_settings` foi removida e remontada como migration única `add_ai_provider_and_chat_run` (rollback local — exige `prisma migrate reset` + `db:migrate`).
- **2026-05-26** — BYO API key & modelo customizado por tracking. `AiSettings` ganhou `aiProvider` (enum `OPENAI|ANTHROPIC|GOOGLE`), `aiModelId`, `aiApiKey` (ciphertext AES-256-GCM) e `aiApiKeyLast4` (só pra UI). Novo utilitário [`src/lib/crypto.ts`](../../lib/crypto.ts) cifra/decifra usando `AI_SECRETS_KEY`. [model.ts](lib/model.ts) ganhou `resolveModel(cfg)` que escolhe entre OpenAI/Anthropic/Google com a key do cliente; quando `cfg` é null/incompleto, cai pro `defaultModel()` (chave NASA). [context.ts](lib/context.ts) decifra a key e expõe `modelConfig` no `AgentContext`; [agent.ts](lib/agent.ts) passou a usar `resolveModel(ctx.modelConfig)` e loga `provider/model/custom` em cada run (sem logar a key). oRPC [get-ai-settings](../../app/router/ia/get-ai-settings.ts) nunca retorna o ciphertext; [update-ai-settings](../../app/router/ia/update-ai-settings.ts) cifra ao salvar, trata `aiApiKey === ""` como limpar, e valida que provider setado tem key disponível. UI: nova aba "Modelo" em [chatbot-ia.tsx](../tracking-settings/components/chatbot-ia.tsx) → [chatbot-ia-model-tab.tsx](../tracking-settings/components/chatbot-ia-model-tab.tsx) com toggle, select de provider+modelo, input password com placeholder `••••••••last4` e botão "Remover". A aba "Geral" não muda. Migração: `add_ai_provider_to_ai_settings`. Fora de escopo: ASTRO command center, transcrição de vídeo, agentes Instagram/Facebook (ainda no n8n).
