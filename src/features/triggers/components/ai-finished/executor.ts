...
import { AiFinishedTriggerFormValues } from "./dialog";

type AiFinishedTriggerData = {
  action?: AiFinishedTriggerFormValues;
};

export const aiFinishedTriggerExecutor: NodeExecutor<
  AiFinishedTriggerData
> = async ({ nodeId, context, step, data }) => {
  const result = await step.run("ai-finished-trigger", async () => {
