              ...
              {isSubmitted ? (
                ...
                blocks.length > 0 && (
                  ...
                  >
                    ...
                    {step === 2 && (
                      <>
                        {/* Barra de progresso "X% preenchido" — atualiza
                            em tempo real conforme o lead responde os
                            campos obrigatórios. Clicar abre a lista dos
                            que faltam, e clicar num item rola até o
                            bloco (que pisca em vermelho). */}
                        <FormFillProgressBar
                        ...
                        {blocks.map((block) => {
                          ...
                          // Bloco fica em ring vermelho pulsante quando
                          // (a) o user clicou em "Enviar" sem preencher
                          //     campo obrigatório, OU
                          // (b) o user clicou no item "faltando" da progress bar.
                          // Como `highlightedIds` traz IDs dos child blocks
                          // (campos), checamos se algum descendant está marcado.
                          const isHighlighted =
                            highlightedIds.has(block.id) ||
                            !!block.childblocks?.some((c) => highlightedIds.has(c.id));
                          return (
                            <div
                              key={block.id}
                              ...
                              className={cn(
                                ...
                                isHighlighted &&
                                  "ring-2 ring-red-500/70 ring-offset-2 ring-offset-transparent animate-pulse",
                              )}
                            >
                              <FormBlockComponent
                                blockInstance={block}
