...
import {
  ...
  StarIcon, PlusIcon, Trash2Icon, LinkIcon, FolderIcon,
} from "lucide-react";
...
interface EventDraft { eventType: string; title: string; scheduledAt: string; durationMinutes: number; meetingLink: string }
interface TaskDraft { title: string; assignedTo: string; priority: string; dueDate: string }
...
export function CampaignPlannerWizard({ open, onOpenChange }: WizardProps) {
  ...
  // clientName derives from org name
  ...
  const reset = () => {
    setStep(0); setCreatedCampaignId(null);
  ...
  const handleNext = async () => {
    if (step === 1 && !createdCampaignId) {
      try {
        const result = await createCampaign.mutateAsync({ title, description, clientName, startDate: startDate || undefined, endDate: endDate || undefined, color, orgProjectId: selectedProjectId ?? undefined });
  ...
  const handleFinish = async () => {
    ...
    for (const ev of events) {
      if (ev.title && ev.scheduledAt) {
        promises.push(createEvent.mutateAsync({ campaignId: createdCampaignId, eventType: ev.eventType as any, title: ev.title, scheduledAt: ev.scheduledAt, durationMinutes: ev.durationMinutes, meetingLink: ev.meetingLink || undefined }));
      }
    }
    ...
    for (const task of tasks) {
      if (task.title) {
        promises.push(createTask.mutateAsync({ campaignId: createdCampaignId, title: task.title, assignedTo: task.assignedTo || undefined, priority: task.priority as any, dueDate: task.dueDate || undefined }));
      }
    }
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) { onOpenChange(false); reset(); } }}>
      <DialogContent className="w-[95vw] max-w-[60rem] max-h-[90vh] overflow-y-auto overflow-x-hidden p-4 sm:p-6">
        ...
        <div className="min-h-[300px] space-y-4 py-2">
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Textarea placeholder="Objetivos, contexto e metas da campanha..." rows={3} value={description} onChange={(e) => setDescription(e.target.value)} />
              </div>
          ...
          {step === 2 && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">Adicione os eventos e datas estratégicas da campanha.</p>
              ...
              <div className="space-y-1.5">
                ...
                <Input placeholder="https://meet.google.com/..." value={newEvent.meetingLink} onChange={(e) => setNewEvent((ev) => ({ ...ev, meetingLink: e.target.value }))} />
              </div>
              <Button variant="outline" size="sm" onClick={addEvent} disabled={!newEvent.title || !newEvent.scheduledAt} className="gap-1">
                <PlusIcon className="size-3.5" /> Adicionar Evento
              </Button>
              {events.length > 0 && (
          ...
          {step === 4 && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">Crie as demandas iniciais da equipe para esta campanha.</p>
              <div className="grid grid-cols-2 gap-3">
              ...
              <Button variant="outline" size="sm" onClick={addTask} disabled={!newTask.title} className="gap-1">
                <PlusIcon className="size-3.5" /> Adicionar Tarefa
              </Button>
              {tasks.length > 0 && (
                <div className="space-y-2">
                  {tasks.map((task, i) => (
