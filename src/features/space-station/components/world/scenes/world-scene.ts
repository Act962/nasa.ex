...
import {
  getDefaultSpriteForUser,
  resolveSpriteUrl,
  resolveRemoteSpriteUrl,
} from "../../../utils/sprite-defaults";
...
export class WorldScene extends (globalThis.Phaser?.Scene ?? class {}) {
  ...
  private placedObjectSprites: Map<string, Phaser.GameObjects.Image> =
    new Map();
  private placedObjectHighlights: Map<string, Phaser.GameObjects.Graphics> =
    new Map();
  /** Corpos de física para objetos com solid:true */
  private placedObjectBodies: Map<string, Phaser.Physics.Arcade.Image> =
    new Map();
  ...
  private areaCollisionBodies: Map<string, Phaser.Physics.Arcade.Image> =
    new Map();
  ...
  private currentTileTool:
    ...
    | "rect-erase"
    | "pan" = "paint";
  private currentTileCell: TileCell = {
    texture: "floor_wood",
    layer: "floor",
    color: "#a0783c",
  };
  ...
  init(
    ...
    this.avatarConfig = data.avatarConfig
      ? ({
          ...data.avatarConfig,
          _photoUrl: data.userImage ?? undefined,
        } as AvatarConfig)
      : data.avatarConfig;
  ...
  create(this: Phaser.Scene & WorldScene) {
    ...
    const elements: WorldElementsConfig = {
      ...DEFAULT_ELEMENTS,
      ...(raw?.elements ?? {}),
    };
    ...
    if (scenario === "tiled") this.renderTiledMap();
      ...
      this.drawStation(elements, rooms, meetingCount);
    ...
    const rawLpcUrl = (
      this.avatarConfig as
        | (AvatarConfig & { lpcSpritesheetUrl?: string })
        | undefined
    )?.lpcSpritesheetUrl;
      ...
      const saved =
        typeof localStorage !== "undefined"
          ? localStorage.getItem(
              `ss:pos:${this.worldConfig?.stationId ?? ""}:${this.localUserId}`,
            )
          : null;
      if (saved) {
    ...
    const onWheel = (ev: WheelEvent) => {
      ...
      const overCanvas =
        ...
        (target instanceof Element &&
          target.closest("#phaser-container") !== null);
    ...
    this.input.keyboard!.on("keydown-D", (ev: KeyboardEvent) => {
      ...
      window.dispatchEvent(
        new CustomEvent("space-station:physics-debug", {
      ...
      try {
        this.game.renderer.snapshot((img) => {
          ...
          if (img.complete && img.naturalWidth > 0) {
            ...
            window.dispatchEvent(
              new CustomEvent("space-station:snapshot-result", {
    ...
    window.addEventListener(
      "space-station:capture-snapshot",
      onCaptureSnapshot,
    );
    this.events.once("shutdown", () => {
    ...
    const onMove = (e: Event) =>
      this.onRemoteMove(
        (e as CustomEvent).detail as { userId: string; x: number; y: number },
      );
    const onLeave = (e: Event) =>
      this.onRemoteLeave((e as CustomEvent).detail as { userId: string });
    ...
    const onTeleportTo = (e: Event) => {
      ...
      window.dispatchEvent(
        new CustomEvent("space-station:player-moved", {
          detail: { x, y },
        }),
      );
    };
    ...
    const onObjectsChanged = (e: Event) => {
      const { objects } = (e as CustomEvent).detail as {
    ...
    const onRequestCenter = () => {
      ...
      window.dispatchEvent(
        new CustomEvent("space-station:center-response", {
          detail: { x, y },
        }),
      );
    };
    const onFocusObject = (e: Event) => {
      ...
      if (id) {
        ...
        if (spr && this.cameras?.main)
          this.cameras.main.pan(spr.x, spr.y, 350, "Sine.easeInOut", false);
      }
    };
    ...
    window.addEventListener(
      "space-station:force-object-depth",
      onForceObjectDepth,
    );
    ...
    const onRoomCfg = (e: Event) => {
      const { config } = (e as CustomEvent).detail as {
    ...
    window.addEventListener(
      "space-station:request-camera-view",
      onRequestCamView,
    );
    ...
    this.input.on("pointerdown", (ptr: Phaser.Input.Pointer) => {
      ...
      const overObj = this.input.manager.hitTest(
        ptr,
        this.children.list as Phaser.GameObjects.GameObject[],
        this.cameras.main,
      );
    ...
    this.input.on("pointerup", (ptr: Phaser.Input.Pointer) => {
      ...
      if (w < 16 || h < 16) return;
      window.dispatchEvent(
        new CustomEvent("space-station:area-drawn", {
          detail: { type, x, y, w, h },
        }),
      );
    });
    ...
    const onTileEditorActive = (e: Event) => {
      ...
      if (active) {
        ...
        if (canvas)
          canvas.style.cursor =
            this.currentTileTool === "pan" ? "grab" : "crosshair";
      } else {
    ...
    const onTileLayerSet = (e: Event) => {
      const { tileLayer } = (e as CustomEvent).detail as {
        tileLayer: TileLayer;
      };
      this.tileLayerState = tileLayer;
    ...
    window.addEventListener(
      "space-station:tile-editor-active",
      onTileEditorActive,
    );
    ...
    this.events.once("shutdown", () => {
      window.removeEventListener(
        "space-station:placed-objects",
        onObjectsChanged,
      );
      window.removeEventListener(
        "space-station:map-editor-active",
        onEditorActive,
      );
      window.removeEventListener(
        "space-station:request-center",
        onRequestCenter,
      );
  ...
  private renderArea(this: Phaser.Scene & WorldScene, area: MapArea) {
    ...
    const label = this.add
      .text(area.x + 6, area.y + 4, `${meta.emoji} ${area.name}`, {
        fontSize: "10px",
        color: "#ffffff",
        backgroundColor: "#00000099",
        padding: { x: 4, y: 2 },
  ...
  private renderAreaHitzone(this: Phaser.Scene & WorldScene, area: MapArea) {
    ...
    hit.on("pointerdown", () => {
      ...
      this.refreshAreaSelection();
      window.dispatchEvent(
        new CustomEvent("space-station:area-selected", {
          detail: { id: area.id },
        }),
      );
    });
    ...
    hit.on("dragend", () => {
      ...
      window.dispatchEvent(
        new CustomEvent("space-station:area-moved", {
  ...
  private syncAreas(this: Phaser.Scene & WorldScene, next: MapArea[]) {
    ...
    this.areaCollisionBodies.forEach((b, id) => {
      if (!nextIds.has(id)) {
        b.destroy();
        this.areaCollisionBodies.delete(id);
      }
    });
    ...
    next.forEach((a) => {
      ...
      if (existing) {
        ...
        if (lbl) {
          lbl.setText(`${meta.emoji} ${a.name}`);
          lbl.setPosition(a.x + 6, a.y + 4);
        }
  ...
  private emitCameraView(this: Phaser.Scene & WorldScene) {
    const cam = this.cameras.main;
    window.dispatchEvent(
      new CustomEvent("space-station:camera-view", {
        detail: {
          x: cam.worldView.x,
          y: cam.worldView.y,
  ...
  private renderPlacedObject(
    ...
    const finalize = () => {
      ...
      img.on("pointerdown", () => {
        ...
        window.dispatchEvent(
          new CustomEvent("space-station:object-selected", {
            detail: { id: obj.id },
          }),
        );
        this.refreshSelectionHighlights();
      ...
      img.on("dragend", () => {
        if (!this.editorActive) return;
        window.dispatchEvent(
          new CustomEvent("space-station:object-moved", {
    ...
    if (this.textures.exists(key)) {
      ...
      this.load.once("loaderror", (file: Phaser.Loader.File) => {
        if (file.key === key)
          console.warn("[WorldScene] failed to load placed object:", obj.url);
      });
      this.load.start();
    }
  }
  ...
  private syncPlacedObjects(
    ...
    this.placedObjectSprites.forEach((spr, id) => {
      if (!nextIds.has(id)) {
        ...
        if (hl) {
          hl.destroy();
          this.placedObjectHighlights.delete(id);
        }
    ...
    switch (cell.texture as TileTextureKey) {
      ...
      case "floor_parquet": {
        ...
        for (let r = 0; r < 4; r++) {
          for (let c = 0; c < 4; c++) {
            if ((r + c) % 2 === 0)
              gfx.fillRect(px + c * 8, py + r * 8 + 7, 8, 1);
      ...
      case "floor_tech": {
        ...
        gfx.beginPath();
        gfx.moveTo(px + 4, py + 16);
        gfx.lineTo(px + 12, py + 16);
        gfx.moveTo(px + 20, py + 16);
        gfx.lineTo(px + 28, py + 16);
        gfx.moveTo(px + 16, py + 4);
        gfx.lineTo(px + 16, py + 12);
        gfx.moveTo(px + 16, py + 20);
        gfx.lineTo(px + 16, py + 28);
        gfx.strokePath();
      ...
      case "floor_brick": {
        ...
        for (let row = 0; row < 4; row++)
          gfx.fillRect(px, py + row * 8 + 7, T, 1);
        for (let row = 0; row < 4; row++) {
          const off = row % 2 === 0 ? 0 : 16;
          for (let c2 = 0; c2 <= 32; c2 += 16)
            gfx.fillRect(px + c2 + off, py + row * 8, 1, 7);
        }
        break;
      }
      case "floor_hex": {
      ...
      case "floor_ice": {
        ...
        gfx.beginPath();
        gfx.moveTo(px + 4, py + 4);
        gfx.lineTo(px + 14, py + 14);
        gfx.moveTo(px + 20, py + 6);
        gfx.lineTo(px + 28, py + 18);
        gfx.moveTo(px + 8, py + 22);
        gfx.lineTo(px + 22, py + 28);
        gfx.strokePath();
      ...
      case "wall_ice": {
        ...
        gfx.beginPath();
        gfx.moveTo(px + 4, py + 15);
        gfx.lineTo(px + 15, py + 22);
        gfx.moveTo(px + 20, py + 18);
        gfx.lineTo(px + 28, py + 28);
        gfx.strokePath();
        break;
      }
      case "wall_cave": {
      ...
      case "deco_water": {
        ...
        for (let x = 0; x < T; x++) {
          ...
          const y2 = Math.round(
            Math.sin((x / T) * Math.PI * 2 + Math.PI) * 2 + 22,
          );
      ...
      case "deco_gravel": {
        for (let i = 0; i < 14; i++) {
          ...
          const gy = (seed * 3) % (T - 3);
          gfx.fillStyle(
            i % 2 === 0 ? darken(color, 0.65) : lighten(color, 1.3),
            1,
          );
  ...
  private floodFillTile(
    ...
    const targetKey = targetCell
      ? `${targetCell.texture}:${targetCell.layer}`
      : "__empty__";
    ...
    while (queue.length > 0) {
      ...
      const existingKey = existing
        ? `${existing.texture}:${existing.layer}`
        : "__empty__";
  ...
  private scheduleSyncTiles(this: Phaser.Scene & WorldScene) {
    ...
    this._tileSyncTimer = setTimeout(() => {
      ...
      if (this.tileLayerState) {
        ...
        window.dispatchEvent(
          new CustomEvent("space-station:tile-layer-changed", {
  ...
  private showResizeHandles(
    ...
    for (const [dx, dy] of corners) {
      const h = this.add
        .rectangle(spr.x + dx, spr.y + dy, 14, 14, 0xffffff)
        .setDepth(200)
      ...
      h.on("drag", (_p: unknown, wx: number, wy: number) => {
        ...
        const newScale = Math.max(
          0.05,
          (dist / this.resizeOrigDist) * this.resizeOrigScale,
        );
        ...
        window.dispatchEvent(
          new CustomEvent("space-station:object-resized", {
      ...
      h.on("dragend", () => {
        this.resizeOrigScale = spr.scale;
        this.resizeOrigDist = Math.hypot(
          spr.displayWidth / 2,
          spr.displayHeight / 2,
        );
        this.repositionResizeHandles(spr);
  ...
  private clearResizeHandles(this: Phaser.Scene & WorldScene) {
    this.resizeHandles.forEach((h) => {
      try {
        ...
        /* ignore destroyed sprites */
      }
    });
    this.resizeHandles = [];
  }

  private repositionResizeHandles(
    this: Phaser.Scene & WorldScene,
    spr: Phaser.GameObjects.Image,
  ) {
    const hw = spr.displayWidth / 2;
  ...
  private setupEditorDOMDrop(active: boolean) {
    ...
    const existing = (canvas as unknown as { __editorDropCleanup?: () => void })
      .__editorDropCleanup;
    if (existing) {
      existing();
      (
        canvas as unknown as { __editorDropCleanup?: () => void }
      ).__editorDropCleanup = undefined;
    }
    if (!active) return;
    ...
    const onDrop = (e: DragEvent) => {
      ...
      const cam = this.cameras.main;
      const worldX =
        cam.worldView.x + (screenX / cam.zoom) * (rect.width / cam.width);
      const worldY =
        cam.worldView.y + (screenY / cam.zoom) * (rect.height / cam.height);
      // Fallback mais robusto usando getWorldPoint
      const wp = cam.getWorldPoint(screenX, screenY);
      window.dispatchEvent(
        new CustomEvent("space-station:object-dropped", {
  ...
  private renderTiledMap(this: Phaser.Scene & WorldScene) {
    ...
    this.add.image(0, 0, "__tiled_bg__").setOrigin(0, 0).setDepth(0);
  ...
  private drawStation(
    ...
    for (let x = 0; x <= WORLD_W; x += T) {
      g.moveTo(x, WALL_T * 2);
      g.lineTo(x, OFFICE_H - WALL_T * 2);
    }
    for (let y = WALL_T * 2; y <= OFFICE_H - WALL_T * 2; y += T) {
      g.moveTo(0, y);
      g.lineTo(WORLD_W, y);
    }
    g.strokePath();
  ...
  private drawRoom(
    ...
    this.add
      .text(
        rx + rw / 2,
        ry + 12,
        `${ROOM_META[type].emoji} ${ROOM_META[type].label}`,
        {
          fontSize: "9px",
          color: "#334455",
    ...
    const ix = rx + WALL_T + 4,
      iy = ry + WALL_T + 4;
    const iw = rw - WALL_T * 2 - 8,
      ih = rh - WALL_T * 2 - 8;
  ...
  private drawRoomInterior(
    ...
    switch (type) {
      case "copa":
        this.drawCopa(g, x, y, w, h);
        break;
      case "cozinha":
        this.drawCozinha(g, x, y, w, h);
        break;
      case "atendimento":
        this.drawAtendimento(g, x, y, w, h);
        break;
      case "reuniao":
        this.drawReuniao(g, x, y, w, h, el);
        break;
      case "recepcao":
        this.drawRecepcao(g, x, y, w, h);
        break;
      case "coworking":
        break;
    }
  ...
  private drawAtendimento(
    ...
    this.add
      .text(x + w * 0.65 + w * 0.125, y + h * 0.15 + T / 2, "ATENDIMENTO", {
        fontSize: "7px",
        color: "#ffffff",
  ...
  private drawReuniao(
    ...
    const tx = x + w / 2,
      ty = y + h / 2;
    const tw = Math.min(w - T * 2, 160),
      th = T * 1.5;
    this.fillRect(g, 0x5d8a9a, tx - tw / 2, ty - th / 2, tw, th);
    g.lineStyle(2, 0x4a7080, 0.8);
    ...
    g.strokeRect(tx - 40, y + 4, 80, T + 8);
  }

  private drawRecepcao(
    g: Phaser.GameObjects.Graphics,
    x: number,
    y: number,
    w: number,
  ...
  private drawSpace(this: Phaser.Scene & WorldScene) {
    ...
    [
      [0.15, 0.2, 400, C.NEBULA1],
      [0.6, 0.4, 500, C.NEBULA2],
      [0.4, 0.75, 380, 0x501020],
      [0.85, 0.15, 300, C.NEBULA1],
    ].forEach(([rx, ry, r, col]) => {
      this.circle(
        ng,
        col as number,
        (rx as number) * WORLD_W,
        (ry as number) * WORLD_H,
        r as number,
        0.18,
      );
    });
    ...
    [
      ...
      const mx = rx * WORLD_W - w / 2,
        my = ry * WORLD_H - h / 2;
      this.fillRect(pg, C.PLATFORM, mx, my, w as number, h as number);
      pg.lineStyle(2, C.PLATFORM_GLOW, 0.6);
      pg.strokeRect(mx, my, w as number, h as number);
  ...
  private drawRocket(this: Phaser.Scene & WorldScene) {
    ...
    for (let y = 0; y < WORLD_H; y += T * 4) {
      for (let x = 0; x < WORLD_W; x += T * 6) {
        ...
        g.lineStyle(1, 0x2a3a5e, 0.4);
        g.strokeRect(x, y, T * 6 - 2, T * 4 - 2);
      }
    }
    const rg = this.add.graphics().setDepth(3);
    // Cockpit
  ...
  private drawLunarBase(this: Phaser.Scene & WorldScene) {
    ...
    // Left module
    const lmodX = domeX - T * 16,
      lmodY = domeY - T;
    this.fillRect(rg, 0x1a2030, lmodX, lmodY, T * 8, T * 4);
    rg.lineStyle(3, 0x3a7abb, 0.8);
    rg.strokeRect(lmodX, lmodY, T * 8, T * 4);
    ...
    // Right module — lab
    const rmodX = domeX + T * 8,
      rmodY = domeY - T;
    this.fillRect(rg, 0x1a1a28, rmodX, rmodY, T * 9, T * 4);
    rg.lineStyle(3, 0x9b30ff, 0.8);
    rg.strokeRect(rmodX, rmodY, T * 9, T * 4);
    ...
    // Solar panels
    [
      [domeX - T * 20, groundY + T * 2],
      [domeX + T * 18, groundY + T * 2],
    ].forEach(([px, py]) => {
      this.fillRect(rg, 0x1a3a5a, px, py, T * 4, T * 1.5);
      rg.lineStyle(1, 0x3a7aaa, 0.7);
      for (let i = 1; i < 4; i++) {
        rg.moveTo(px + T * i, py);
        rg.lineTo(px + T * i, py + T * 1.5);
      }
      rg.strokePath();
    });
  ...
  private drawMissionControl(this: Phaser.Scene & WorldScene) {
    ...
    for (let r = 0; r <= MH; r++) {
      rg.moveTo(0, r * T);
      rg.lineTo(WORLD_W, r * T);
    }
    for (let c = 0; c <= MW; c++) {
      rg.moveTo(c * T, 0);
      rg.lineTo(c * T, WORLD_H);
    }
    rg.strokePath();
    ...
    rg.lineStyle(4, 0x00d4ff, 0.9);
    rg.strokeRect(T * 2, scrY + T, WORLD_W - T * 4, scrH - T * 2);
    // Screen content — world map grid
    rg.lineStyle(1, 0x003355, 0.6);
    for (let i = 0; i < 20; i++) {
      rg.moveTo(T * 2, scrY + T + i * T * 0.5);
      rg.lineTo(WORLD_W - T * 2, scrY + T + i * T * 0.5);
      rg.moveTo(T * 2 + i * T * 3.8, scrY + T);
      rg.lineTo(T * 2 + i * T * 3.8, scrY + scrH - T);
    }
    rg.strokePath();
    // Blips on screen
    [
      [WORLD_W * 0.25, scrY + scrH * 0.4],
      [WORLD_W * 0.5, scrY + scrH * 0.6],
      [WORLD_W * 0.7, scrY + scrH * 0.3],
      [WORLD_W * 0.85, scrY + scrH * 0.5],
    ].forEach(([bx, by]) => {
      this.circle(rg, 0x00ff88, bx, by, 5, 0.9);
      rg.lineStyle(1, 0x00ff88, 0.3);
      rg.strokeCircle(bx, by, 14);
    });
    // Screen labels
    ...
    for (let row = 0; row < 4; row++) {
      ...
      rg.lineStyle(2, 0x1a3a5a, 0.8);
      rg.strokeRect(T * 2, ry, WORLD_W - T * 4, T * 3);
      // Individual stations
    ...
    [
      ...
      for (let i = 0; i < 6; i++) {
        ...
        this.circle(
          rg,
          i % 3 === 0 ? 0x00ff88 : i % 3 === 1 ? 0xff3344 : 0x0088ff,
          px + T,
          py2 + T * 1.5,
          8,
          0.9,
  ...
  private drawLab(this: Phaser.Scene & WorldScene) {
    ...
    const bench = (bx: number, by: number, w: number) => {
      ...
      rg.lineStyle(2, 0x3a5a7a, 0.8);
      rg.strokeRect(bx, by, w, T * 1.5);
      ...
      for (let e = 0; e < Math.floor(w / (T * 3)); e++) {
        ...
        rg.lineStyle(1, 0x00d4ff, 0.5);
        rg.strokeRect(ex + T * 1.3, by + T * 0.3, T * 1.2, T);
      }
    };
    ...
    const cX = WORLD_W * 0.75,
      cY = WORLD_H * 0.5;
    this.circle(rg, 0x0a0a18, cX, cY, T * 5, 0.95);
    rg.lineStyle(6, 0x9b30ff, 0.8);
    rg.strokeCircle(cX, cY, T * 5);
    rg.lineStyle(3, 0x9b30ff, 0.4);
    rg.strokeCircle(cX, cY, T * 4);
    rg.lineStyle(2, 0x9b30ff, 0.2);
    rg.strokeCircle(cX, cY, T * 3);
    ...
    for (let i = 0; i < 6; i++) {
      const px = WORLD_W - T * 5,
        py = T * (3 + i * 7);
      this.fillRect(rg, 0x0a1a2a, px, py, T * 3, T * 5);
      rg.lineStyle(3, 0x00d4ff, 0.8);
      rg.strokeRect(px, py, T * 3, T * 5);
      this.circle(rg, 0x003355, px + T * 1.5, py + T * 2, T * 1.2, 0.6);
      rg.lineStyle(2, 0x00d4ff, 0.3);
      rg.strokeCircle(px + T * 1.5, py + T * 2, T * 1.2);
    }
  }
  ...
  private drawHangar(this: Phaser.Scene & WorldScene) {
    ...
    for (let r = 0; r < MH; r++)
      for (let c = 0; c < MW; c++) {
        if (r % 4 === 0 || c % 4 === 0)
          this.fillRect(rg, 0x222228, c * T, r * T, T, T);
      }

    // Floor markings — yellow lane lines
    rg.lineStyle(4, 0xffd700, 0.7);
    for (let i = 1; i < 5; i++) {
      rg.moveTo(0, WORLD_H * (i / 5));
      rg.lineTo(WORLD_W, WORLD_H * (i / 5));
    }
    rg.strokePath();
    rg.lineStyle(4, 0xffd700, 0.7);
    rg.moveTo(WORLD_W / 2, 0);
    rg.lineTo(WORLD_W / 2, WORLD_H);
    rg.strokePath();
    ...
    const ship = (sx: number, sy: number, scale: number = 1) => {
      ...
      rg.lineStyle(3, 0x4a6a8a, 1);
      rg.strokeRect(sx - w / 2, sy - h / 2, w, h);
      // Cockpit
      this.circle(rg, 0x3a6a9a, sx - w * 0.35, sy, h * 0.4, 0.9);
      rg.lineStyle(2, 0x70c0f0, 0.7);
      rg.strokeCircle(sx - w * 0.35, sy, h * 0.4);
      // Engine pods
      [
        [sx + w * 0.35, sy - h * 0.4],
        [sx + w * 0.35, sy + h * 0.4],
      ...
      [
        ...
        rg.lineStyle(4, 0x3a3a4a, 1);
        rg.moveTo(lx, ly);
        rg.lineTo(lx, ly + T * 1.5 * scale);
        rg.strokePath();
    ...
    for (let i = 0; i < 10; i++) {
      ...
      rg.lineStyle(2, 0xffd700, 0.5);
      rg.strokeRect(cx, cy, T * 3, T * 2);
    }
    for (let i = 0; i < 10; i++) {
      const cx = T * (3 + i * 7),
        cy = WORLD_H - T * 3;
    ...
    const doorW = WORLD_W * 0.6,
      doorX = (WORLD_W - doorW) / 2;
  ...
  private drawMars(this: Phaser.Scene & WorldScene) {
    ...
    for (let i = 0; i < 120; i++) {
      const rx = Math.random() * WORLD_W,
        ry = T * 10 + Math.random() * (WORLD_H - T * 10);
    ...
    const dome = (
      ...
      rg.lineStyle(4, 0x4a8abb, 0.7);
      rg.strokeCircle(dx, dy, r);
      rg.lineStyle(2, 0x70c0f0, 0.3);
      rg.strokeCircle(dx, dy, r - 6);
  ...
  private drawObservatory(this: Phaser.Scene & WorldScene) {
    ...
    for (let i = 0; i < 300; i++) {
      ...
      const by =
        WORLD_H * 0.1 +
        Math.sin((bx / WORLD_W) * Math.PI) * WORLD_H * 0.15 +
        Math.random() * T * 4 -
        T * 2;
    ...
    // Nebula
    rg.fillStyle(0x3a0a60, 0.15);
    rg.fillCircle(WORLD_W * 0.3, WORLD_H * 0.2, 200);
    rg.fillStyle(0x0a1a60, 0.15);
    rg.fillCircle(WORLD_W * 0.7, WORLD_H * 0.15, 150);
    ...
    const obsX = WORLD_W / 2,
      obsY = hillY;
    this.circle(rg, 0x1a1a28, obsX, obsY, 140, 0.97);
    rg.lineStyle(5, 0xaaaacc, 0.8);
    rg.strokeCircle(obsX, obsY, 140);
    // Dome slit
    rg.lineStyle(8, 0x050510, 1);
    rg.moveTo(obsX, obsY - 140);
    rg.lineTo(obsX, obsY + 10);
    rg.strokePath();
    // Telescope barrel
    this.fillRect(rg, 0x2a2a3e, obsX - T * 0.5, obsY - T * 5, T, T * 5);
    this.circle(rg, 0x3a3a50, obsX, obsY - T * 4.5, T * 0.8, 1);
    rg.lineStyle(3, 0xaaaacc, 0.5);
    rg.strokeCircle(obsX, obsY - T * 4.5, T * 0.8);

    // Building base
    this.fillRect(rg, 0x151520, obsX - T * 5, hillY, T * 10, T * 4);
    rg.lineStyle(3, 0x3a3a50, 0.8);
    rg.strokeRect(obsX - T * 5, hillY, T * 10, T * 4);
    ...
    [
      ...
      rg.lineStyle(2, 0x2a2a40, 0.8);
      rg.strokeRect(bx, by, T * 7, T * 5);
      // Lit windows
      for (let w = 0; w < 2; w++) {
        this.fillRect(
          rg,
        ...
        rg.lineStyle(1, 0xffa040, 0.7);
        rg.strokeRect(bx + T + w * T * 3, by + T, T * 1.5, T * 1.2);
      }
    });
    ...
    [
      ...
      rg.lineStyle(3, 0x4a4a60, 1);
      rg.moveTo(dx, dy);
      rg.lineTo(dx, dy + T * 2);
      rg.strokePath();
      rg.lineStyle(2, 0x6a6a80, 0.8);
      rg.strokeCircle(dx, dy - T * 0.5, T * 1.5);
      rg.lineStyle(1, 0x6a6a80, 0.4);
      rg.strokeCircle(dx, dy - T * 0.5, T);
    });

    // Path up the hill
    rg.lineStyle(T * 1.5, 0x181820, 0.8);
    rg.moveTo(obsX, hillY + T * 4);
    rg.lineTo(obsX - T * 10, WORLD_H);
    rg.moveTo(obsX, hillY + T * 4);
    rg.lineTo(obsX + T * 10, WORLD_H);
    rg.strokePath();
  }
  ...
  private drawBridge(this: Phaser.Scene & WorldScene) {
    ...
    for (let r = 0; r <= MH; r += 4) {
      rg.moveTo(0, r * T);
      rg.lineTo(WORLD_W, r * T);
    }
    for (let c = 0; c <= MW; c += 4) {
      rg.moveTo(c * T, 0);
      rg.lineTo(c * T, WORLD_H);
    }
    rg.strokePath();
    ...
    rg.lineStyle(6, 0x2a5a8a, 1);
    rg.strokeRect(T * 2, vpY, WORLD_W - T * 4, vpH);
    // Window frame struts
    rg.lineStyle(4, 0x1a2a3a, 1);
    for (let i = 1; i < 6; i++) {
      rg.moveTo(WORLD_W * (i / 6), vpY);
      rg.lineTo(WORLD_W * (i / 6), vpY + vpH);
    }
    rg.strokePath();
    ...
    rg.lineStyle(3, 0x2a4a6a, 0.9);
    rg.strokeRect(T * 20, platY, T * 40, T * 6);
    // Chair
    this.circle(rg, 0x1a1a2e, WORLD_W / 2, platY + T * 3, T * 2, 0.9);
    rg.lineStyle(2, 0x00d4ff, 0.6);
    rg.strokeCircle(WORLD_W / 2, platY + T * 3, T * 2);
    this.fillRect(rg, 0x0a0a18, WORLD_W / 2 - T, platY + T, T * 2, T * 4); // back
    // Armrest consoles
    [
      [WORLD_W / 2 - T * 3, platY + T * 2],
      [WORLD_W / 2 + T * 1.5, platY + T * 2],
    ].forEach(([ax, ay]) => {
      this.fillRect(rg, 0x0e1e2e, ax, ay, T * 1.5, T * 2);
      this.fillRect(rg, 0x003344, ax + T * 0.2, ay + T * 0.3, T, T * 0.8);
      rg.lineStyle(1, 0x00d4ff, 0.5);
      rg.strokeRect(ax + T * 0.2, ay + T * 0.3, T, T * 0.8);
    });

    // Helm / nav console
    const helmY = platY + T * 7;
    this.fillRect(rg, 0x0a1428, T * 25, helmY, T * 30, T * 3);
    rg.lineStyle(3, 0x00d4ff, 0.7);
    rg.strokeRect(T * 25, helmY, T * 30, T * 3);
    for (let b = 0; b < 12; b++) {
      const bx = T * 26 + b * T * 2.3;
      this.fillRect(rg, 0x001a33, bx, helmY + T * 0.5, T * 2, T * 1.5);
      rg.lineStyle(1, 0x0088ff, 0.4);
      rg.strokeRect(bx, helmY + T * 0.5, T * 2, T * 1.5);
    }
    ...
    [
      ...
      rg.lineStyle(2, 0x1a3a5a, 0.8);
      rg.strokeRect(sx, sy, T * 10, T * 8);
      for (let sc = 0; sc < 3; sc++) {
        this.fillRect(rg, 0x001a33, sx + T + sc * T * 3, sy + T, T * 2, T * 2);
        rg.lineStyle(1, 0x0088ff, 0.4);
        rg.strokeRect(sx + T + sc * T * 3, sy + T, T * 2, T * 2);
      }
    ...
    rg.lineStyle(3, 0x2a2a3a, 0.8);
    rg.strokeRect(0, engY, WORLD_W, T * 8);
    for (let i = 0; i < 8; i++) {
      const ex = T * (4 + i * 9);
      this.fillRect(rg, 0x0a1820, ex, engY + T, T * 6, T * 5);
      rg.lineStyle(2, 0x00d4ff, 0.4);
      rg.strokeRect(ex, engY + T, T * 6, T * 5);
      // Engine bar graphs
  ...
  private loadLpcSpritesheet(
    ...
    const profileUrl = (
      this.avatarConfig as AvatarConfig & { _photoUrl?: string }
    )?._photoUrl;

    const isAlive = () =>
      this.scene?.isActive?.() ??
      !(this.game as Phaser.Game & { isDestroyed?: boolean })?.isDestroyed;
  ...
  private createGalaxyPortal(this: Phaser.Scene & WorldScene) {
    ...
    const portal = this.add
      .circle(px, py, 28, 0x7c3aed, 0.85)
      .setDepth(15)
      .setInteractive();
    this.add.circle(px, py, 36, 0x818cf8, 0.2).setDepth(14);
    this.add
      .text(px, py + 42, "🌌 Galáxia", { fontSize: "9px", color: "#c4b5fd" })
      .setOrigin(0.5)
      .setDepth(15);
    portal.on("pointerdown", () =>
      window.dispatchEvent(new CustomEvent("space-station:open-galaxy")),
    );
    this.tweens.add({
      targets: portal,
      alpha: { from: 0.6, to: 1 },
  ...
  private drawDesk(
    ...
    if (type === "space") {
      ...
      if (w > 80) {
        this.fillRect(g, C.MONITOR, x + w - 28, y + 4, 18, 13);
        this.fillRect(g, C.MONITOR_GLOW, x + w - 26, y + 6, 14, 9, 0.5);
      }
    }
  ...
  private onRemoteJoin(data: {
    ...
    if (process.env.NODE_ENV === "development") {
      console.debug(
        "[WorldScene] creating remote player:",
        data.userId,
        "name:",
        data.name,
      );
  ...
  private loadRemoteSprite(
    ...
    const applyTexture = (canvas: HTMLCanvasElement) => {
      ...
      this.textures.addSpriteSheet(texKey, canvas as any, {
        ...
        startFrame: 0,
        endFrame:
          Math.floor(canvas.width / fw) * Math.floor(canvas.height / fw) - 1,
      });
      ...
      const spr = this.add
        .sprite(curX, curY, texKey, frameKey)
        .setDepth(20)
  ...
  private applyAvatarScale(this: Phaser.Scene & WorldScene, scale: number) {
    ...
    const body = this.lpcSprite.body as Phaser.Physics.Arcade.Body | undefined;
    if (!body) return;
    if (isPipoya) {
      body
        .setSize(20 * userScale, 28 * userScale)
        .setOffset(6 * userScale, 4 * userScale);
    } else {
      body
        .setSize(24 * userScale, 40 * userScale)
        .setOffset(20 * userScale, 22 * userScale);
    }
  }
  ...
  private setZoom(this: Phaser.Scene & WorldScene, value: number) {
    ...
    window.dispatchEvent(
      new CustomEvent("space-station:zoom-changed", {
      ...
      if (moving) {
        ...
        if (this.animTimer % 10 === 0) {
          ...
          this.player.setTexture(
            this.animFrame === 0 ? "__player_walk_a__" : "__player_walk_b__",
          );
    ...
    this.remotePlayers.forEach((rp, userId) => {
      ...
      if (Math.sqrt(dx * dx + dy * dy) <= r) {
        ...
        if (!this.nearbyPeers.has(userId)) {
          // entered proximity
          window.dispatchEvent(
            new CustomEvent("space-station:proximity-enter", {
              detail: {
                peerId: userId,
  ...
  private checkAreaTriggers(this: Phaser.Scene & WorldScene) {
    ...
    for (const id of this.insideAreaIds) {
      if (!nowInside.has(id)) {
        ...
        if (area) {
          window.dispatchEvent(
            new CustomEvent("space-station:area-leave", {
  ...
  private onAreaEnter(this: Phaser.Scene & WorldScene, area: MapArea) {
    window.dispatchEvent(
      new CustomEvent("space-station:area-enter", {
    ...
    if (area.type === "info" && area.props?.message) {
      window.dispatchEvent(
        new CustomEvent("space-station:show-info", {
    ...
    if (area.type === "website" && area.props?.url) {
      window.dispatchEvent(
        new CustomEvent("space-station:open-website", {
