...
| `StarsPayment` | Checkout intermediário p/ gateways — `status` (pending/paid/failed/expired), `provider`, `externalId`, `metadata`. |
...
### 5.1. Top-up avulso (pacote)
1. `orpc.stars.listPackages` lista `StarPackage` ativos.
2. Usuário escolhe pacote em `StarsPurchaseModal`.
3. `orpc.stars.createGatewayCheckout` cria `StarsPayment` (status `pending`) e devolve `checkoutUrl` (Stripe) ou `pixQrCode`/`pixPayload` (Asaas).
4. Webhook [src/app/api/stripe/webhook/route.ts:110-129](../src/app/api/stripe/webhook/route.ts) recebe `checkout.session.completed`, marca `StarsPayment.status = "paid"`, chama `purchaseTopUp(orgId, packageId)` e dispara `processPaymentPartnerEffects` (comissão NASA Partner).

> Existe também um fluxo legacy (`createCheckoutSession` + `purchasePackage`) — só Stripe, sem Asaas.
...
- `createGatewayCheckout` aplica snapshot de pricing de partner antes de criar a sessão (lookup em `processPaymentPartnerEffects`).
...
| `createGatewayCheckout` | Checkout multi-gateway (atual) |
...
> ⚠️ Os hooks client (`useStars...`) **ainda não estão em `src/features/stars/hooks/`**. Hoje os componentes importam `orpc` direto — fora do padrão definido no [CLAUDE.md](../CLAUDE.md#9). Refator pendente.
...
- `StarsPurchaseModal` — fluxo de compra de pacote (escolhe pacote → método → checkout)
...
- `checkout.session.completed` — roteia por `metadata.itemType`:
  - `course_public_purchase` → marca `PendingCoursePurchase` paga
  - top-up (stars gateway) → `purchaseTopUp` + partner effects
  - `plan` → vincula plano à org + `runMonthlyCycle` (primeira vez)
  - top-up legacy → `purchaseTopUp`
- `invoice.payment_succeeded` — **TODO**: mapear customer → org e rodar ciclo
- `customer.subscription.deleted` — **TODO**: remover plano da org

Asaas webhook: não localizado nesta auditoria — verificar antes de assumir que renovação por pix/boleto recredita.
