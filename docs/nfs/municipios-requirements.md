...
- **2026-07-09** — Bloqueio da Focus para Teresina (seção 8) foi o motivo direto para adotar a **NFE.io** como gateway padrão (ver `docs/nfs/fiscal-ddd-roadmap.md`): a NFE.io resolve municipal/nacional internamente (sem a distinção MUNICIPAL/NACIONAL exposta ao usuário) e não depende do mesmo convênio da Focus. `resolveMunicipioRequirements` passou a ser reusado também pelo preflight NFE.io (`src/features/fiscal/lib/gateways/adapters/nfe-io/build-service-invoice-payload.ts`) para os mesmos requisitos de CNAE e endereço do tomador — o registry é agora gateway-agnóstico. A Focus continua registrada (`FiscalGateway.FOCUS_NFE`) para notas antigas, mas o formulário fiscal do Forge não oferece mais o seletor de gateway — toda empresa nova é sincronizada na NFE.io.
