...
| Data | Mudança |
| --- | --- |
| 2026-09-15 | **Envio outbound por organização (spec 0017 — lembretes com boleto).** Novo [`providers/send-org-document.ts`](../src/features/tracking-chat/lib/providers/send-org-document.ts): `sendOrganizationWhatsAppDocument`/`sendOrganizationWhatsAppText` escolhem a instância `CONNECTED` mais antiga da org e enviam via `resolveOutboundProvider` + `sendMedia`/`sendText` da PORT (sem mudança na PORT nem nos adapters). Documento vai por link pré-assinado de 15 min. Na Meta, fora da janela de 24h o envio falha e o motivo fica em `PaymentReminder.deliveryLog`. Consumido pelo Inngest `payment-reminder-fire`. Branch: `feature/W-astro-finance-tools-20260915`. |
| 2026-09-15 | **Astro-bot aceita documento/imagem nos dois webhooks (spec 0019).** Intercept do Astro pelo WhatsApp estendido de texto puro para `document`/`image` (Meta: POST com 1 mensagem; Uazapi: `DocumentMessage`/`ImageMessage`), só de membro allow-listado em org com `OrganizationBotConfig.financeEnabled`. Mudança aditiva: caminho de texto, echo-suppression Uazapi e pipeline canônica intocados; qualquer erro antes de `handled: true` cai no `catch` e segue o atendimento normal. Download pelo client cru (`downloadInboundMedia` Meta / `downloadFile` Uazapi) em `src/features/astro-bot/lib/inbound-media.ts`, gravando `PaymentAttachment` (`sourceChannel: "whatsapp"`). Sem mudança na PORT nem nos adapters. Migration `bot_config_finance_enabled` (coluna aditiva). Confirmação de proposta segue por texto SIM/NÃO — a PORT não envia `interactive`. Branch: `feature/W-astro-finance-tools-20260915`. |
