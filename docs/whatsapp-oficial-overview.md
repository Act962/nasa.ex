...
| [send-marketing-message.ts](../src/http/whats-oficial/send-marketing-message.ts) | `sendMarketingMessage(accessToken, phoneNumberId, input)` — `POST /{phone_number_id}/marketing_messages` (MM API) type `template` + `product_policy: "CLOUD_API_FALLBACK"`. Espelha `send-template.ts`; só templates de **marketing aprovados**. **Função pura — abre o app de Campanhas (`docs/campanhas-overview.md`); só é chamada de fato na Fase 3 desse app.** |
| [http/whats-oficial/types.ts](../src/http/whats-oficial/types.ts) | `MessageTemplate`, `TemplateComponent`, `MessageTemplatesResponse`, `SendTemplateInput`, `SendMarketingMessageInput`, `MarketingMessageProductPolicy`. |
