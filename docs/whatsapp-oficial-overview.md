...
> Documento vivo da integração com a **API Oficial do WhatsApp Business (Meta Cloud API)** no NASA. Última revisão: 2026-06-25 (**Fase 9** — Templates HSM listar+enviar via fetch ao vivo, status ticks #4, gate webhook Uazapi #9, janela de 24h só pra `META_CLOUD`. Migration aditiva `20260625000000_add_message_status_delivered_failed`).
>
...
| [http/whats-oficial/types.ts](../src/http/whats-oficial/types.ts) | `MessageTemplate`, `TemplateComponent`, `MessageTemplatesResponse`, `SendTemplateInput`. |
...
| Data | Mudança |
| --- | --- |
| 2026-07-02 | **Melhorias de robustez/UX — followups #10, #12, #14, #15, #18 fechados.** **#12 (🔴 high — envio pro número errado):** `normalizePhoneToMetaE164` agora só insere o 9º dígito BR quando o 1º dígito do assinante é `8`/`9` (mobile) — fixos BR de 12 dígitos (`55 DD 3XXXXXXX`) não ganham mais `9` espúrio que gerava 131030 ou entrega ao número errado ([normalize-phone.ts](../src/features/tracking-chat/lib/providers/adapters/meta-cloud/normalize-phone.ts)). **#18 (revenue leak):** `create-with-sticker.ts` passou a chamar `chargeMessageOutbound` (mediaType `image`, alinhado com insights) DEPOIS do resolver e ANTES do send — figurinha antes saía de graça. **#14 (🔴 high):** novo helper `mapOutboundError(err, errors)` em [`outbound-errors.ts`](../src/features/tracking-chat/lib/providers/outbound-errors.ts) traduz `OutboundProviderError` (instância ausente, credenciais Meta incompletas, resposta inválida do provider, janela 24h) em `BAD_REQUEST { data: { code, feature?, fields? } }` estruturado — aplicado nos 7 handlers `create*` (via outer catch) + `create-with-sticker` (wrap do resolve + inner send catch). Antes esses erros viravam 500 opaco. **#15 (🟡 medium):** novo `serializeOutboundError(err)` preserva `code`/`feature` por destino no `forward.ts` (sucesso parcial via `Promise.allSettled`) em vez de achatar em `String(err)`. **#10 (UX):** gating preventivo — novo hook `useWhatsAppFeatureGate(trackingId)` ([tracking-chat/hooks](../src/features/tracking-chat/hooks/use-whatsapp-feature-gate.ts)); `message-box` passa `metaUnsupported` pro `selected-message-options`, que desabilita "Editar"/"Apagar" com o motivo quando `provider=META_CLOUD`; `footer-chat` esconde o item "Botões" pra Meta. `useWhatsAppProviderSettings` ganhou opção `{ enabled }` (query deduplicada por `trackingId` — 1 request pra toda a lista de mensagens). Typecheck limpo (só resta erro pré-existente de `mediabunny` não instalado, arquivo não tocado). Branch: `feature/tracking-whatsapp-oficial-quickwins-20260702`. |
...
| 10 | ~~**UI gating preventivo de features Meta-unsupported**~~ — **fechado em 2026-07-02.** | — | Novo hook `useWhatsAppFeatureGate(trackingId)` (embrulha `useWhatsAppProviderSettings`, agora com `{ enabled }`). `message-box` passa `metaUnsupported`/`unsupportedReason` pro `selected-message-options`, que renderiza "Editar"/"Apagar" **desabilitados com o motivo** quando `provider=META_CLOUD`. `footer-chat` esconde o item "Botões" pra Meta. Query deduplicada por `trackingId` (1 request pra toda a lista). |
| 23 | **Endurecer envio de figurinha pela API Oficial (WebP/tamanho + cobrança)** | Antes de promover figurinha pra produção em trackings `META_CLOUD`, OU se surgir ticket "figurinha não enviou na Oficial" / "cobrou ★ e não entregou". | **Contexto:** o contrato de envio de sticker está 100% conforme a doc Meta (`type:"sticker"`, `sticker:{ id \| link }`, sem caption — ver [send-media.ts](../src/http/whats-oficial/send-media.ts)). Faltam só robustezas operacionais: **(a)** a Meta exige **WebP** (`.webp`/`image/webp`), estático **≤100KB** / animado **≤500KB** — hoje [`stickers/create.ts`](../src/app/router/stickers/create.ts) só confia no `mimetype` que o client manda (default `image/webp`), sem validar o arquivo real nem o tamanho; sticker inválido só falha no envio. Melhoria: validar formato+tamanho na criação (HEAD no R2) e rejeitar cedo. **(b)** a doc recomenda **`id` (upload prévio via `/media`) em vez de `link`** ("not recommended") — hoje mandamos a URL do R2 → `{ link }`; dá pra usar `uploadOfficialMedia` (Fase 1) e enviar `id` (1 ida extra à Graph por envio). **(c)** com o followup #18, o ★ é cobrado **antes** do send — sticker rejeitado por formato = cliente pagou e não entregou; considerar cobrar só após `sendMedia` confirmar (ou estornar em falha) **só** no caminho sticker. Estimativa: ~40 linhas de validação + decisão de produto sobre cobrança. |
...
| 12 | ✅ parcialmente fechado em 2026-07-02 | ~~**`normalizePhoneToMetaE164` corrompe fixos BR de 12 dígitos**~~ | — | Tightening aplicado em `normalize-phone.ts`: só insere `9` quando `digits[4] === "8" \|\| digits[4] === "9"` (mobile BR antigo). Fixos (2-5) passam sem alteração. **Pendente (baixo impacto):** validação de phone no Zod dos forms `createLead`/`createLeadWithTags`/import CSV pra rejeitar < 13 dígitos com country code 55 — followup separado se surgir lead manual com número curto. |
...
| 14 | ✅ fechado em 2026-07-02 | ~~**Erros do resolver retornam 500 em vez de BAD_REQUEST estruturado**~~ | — | Novo helper `mapOutboundError(err, errors): never` em `outbound-errors.ts` mapeia qualquer `OutboundProviderError` (incl. subclasses com `feature`/`fields`) pra `errors.BAD_REQUEST({ data: { code, feature?, fields? } })`; outros erros repassados intactos. Aplicado nos 7 `create*` (outer catch) + `create-with-sticker` (wrap do resolve + inner send catch). Exportado pelo barrel `@/features/tracking-chat/lib/providers`. |
| 15 | ✅ fechado em 2026-07-02 | ~~**`forward.ts` perde `code` do erro via `String(err)`**~~ | — | Novo `serializeOutboundError(err): { message, code?, feature? }` em `outbound-errors.ts` preserva `code`/`feature` do `OutboundProviderError`; `forward.ts` devolve esses campos por destino no array de `results` (sucesso parcial). Frontend pode renderizar conforme `code`. |
...
| 18 | ✅ fechado em 2026-07-02 | ~~**`create-with-sticker.ts` não chama `chargeMessageOutbound`**~~ | — | `chargeMessageOutbound({ organizationId, userId, channel: "whatsapp", mediaType: "image" })` adicionado DEPOIS do `resolveOutboundProvider` e ANTES do send (ordem do Fix #13/#2 — não cobra se o resolver lançar). `mediaType: "image"` alinha com a categorização de insights do sticker (o `mediaType` do charge é só descrição; o enum não tem "sticker"). |
